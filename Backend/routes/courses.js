...
courses.post("/getquizscore",(req,res)=>{
    ...
    (async()=>{
        try {
            ...
            console.log(result);
            let quizsub=result.quiz.filter((data)=>{
                return data._id==qid;
            })
            console.log("==========>");
            console.log(qid);
...
courses.post("/",requireAuth,(req,res)=>{
  
    let {role,email:_id}=req.body;
...
(async()=>{
    ...
    if(searchTerm==="courseId"){
       ...
       switch(filterOption){
           case ">": 
            results=await courseModel.find({courseId:{$gte:courseId}})
           break;
           case "<":
       results=await courseModel.find({courseId:{$lte:courseId}})
           break;
           case "=":
      results=await courseModel.find({courseId}) 
      break;
       }
      let result=results.map((data)=>{
        ...
        console.log(query);
       let results=await courseModel.find(query);
       let result=results.map((data)=>{
        return cidMutator(data);
      });
...
courses.post("/create",(req,res)=>{
    ...
    if(result){
        res.status(200).json({message:"Course Id Already exists !"});
    }else{
